document.addEventListener('DOMContentLoaded', () => {
  ...
  // Contact Form Floating Label Support (checking placeholder)
  const inputs = document.querySelectorAll('.form-group-float input, .form-group-float textarea');
  inputs.forEach(input => {
    // Add placeholder=" " to inputs to support CSS :placeholder-shown trick
    input.setAttribute('placeholder', ' ');
  });

  
  // Kanban Task Mover logic
  const columns = document.querySelectorAll('.mock-column');
  const tasks = document.querySelectorAll('.mock-task-card');

  function updateColumnCounts() {
    columns.forEach(col => {
      const countSpan = col.querySelector('.col-count');
      const cardsCount = col.querySelectorAll('.mock-task-card').length;
      if (countSpan) {
        countSpan.textContent = cardsCount;
      }
    });
  }
  ...
  if (columns.length > 0 && tasks.length > 0) {
    tasks.forEach(task => {
      task.addEventListener('click', () => {
        ...
        const currentIdx = Array.from(columns).indexOf(currentColumn);
        const nextIdx = (currentIdx + 1) % columns.length;
        
        // Move task to next column
        columns[nextIdx].appendChild(task);
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      alert('¡Gracias por registrarte en NexoFlow! Te enviaremos un correo con las instrucciones de acceso en los próximos 5 minutos.');
      form.reset();
    });
  }
});
