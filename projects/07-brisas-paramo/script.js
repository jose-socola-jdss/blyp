document.addEventListener('DOMContentLoaded', () => {
  ...
  // Transparent Nav transition to solid on scroll
  ...
  // Stay Booking & Carbon Offset Calculator logic
  ...
  if (stayNights && carbonOffset && ecoTrees) {
    stayNights.addEventListener('input', () => {
      const nights = parseInt(stayNights.value) || 0;
      const co2 = nights * 20; // 20kg per night
      const trees = Math.ceil(nights * 1);
      carbonOffset.innerText = `${co2} kg de CO₂ compensados`;
      ecoTrees.innerText = `Equivalente a patrocinar la siembra de ${trees} árbol${trees > 1 ? 'es' : ''} nativo${trees > 1 ? 's' : ''}.`;
    });
  }
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      alert('¡Gracias por escribirnos! Te responderemos para confirmar disponibilidad y tarifas especiales en menos de 12 horas.');
      form.reset();
    });
  }
});
